...
4. 直接輸入雙方比分，按「更新比分」或在第二格按 Enter。可一次跳多分或修正錯誤；「復原上次更新」會一次還原雙方比分。達到局末分數後按「確認完賽」，再開下一場。
5. 「播報」輸入現場文字，觀眾會即時收到。
...
- 使用兩個大字比分輸入框與一個「更新比分」按鈕，手機開啟數字鍵盤。背景同步保留輸入內容，儲存與錯誤提示顯示於管理視窗內。
...
- `runTransaction` 確認提交後立即更新管理頁的已儲存版本；晚到的舊監聽資料不能使比分倒退。
- 手動比分比較本場原比分與狀態，其他播報更新不會阻擋計分；同場被另一人修改時，保留草稿並提供「載入最新比分」。其他表單仍檢查 revision。離線交易不會排隊成為已儲存比分。
- 同步期間可先輸入下一筆，前一筆完成後再按「更新比分」，無需重新整理。離線時停用送出，保留目前輸入內容。
